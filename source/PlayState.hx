package; // hi whoever reads this. first i wanted to say thanks for playing my mod, and if you came to this file to check out how all of the stuff in this mod was done, well, then it makes me happy to think i've inspired someone to wanna do something like this, im no coder, im not good at it at all but im really proud of this mod and i hope you enjoy it. thanks again, you're amazing    -swizik
...
class PlayState extends MusicBeatState
{
	...
	var P2sSKY:FlxSprite;
	var P2trees:FlxSprite;
	var P2bg:FlxSprite;
	var P2bg2:FlxSprite;
	var mercs:FlxSprite;
	var codes:FlxSprite;
	...
	override public function create()
	{
		...
		if (curSong != SONG.song)
		{
			...
			else if (SONG.song.toLowerCase() == 'you-cant-run')
			{
				...
				remove(dad);
				dad = new Character(100, 100, 'demo-charge');
				add(dad);
				...
				gf = new Character(100, 100, 'gf-blue');
				add(gf);
				remove(gf);
				
				boyfriend = new Boyfriend(100, 100, 'bf-running');
				add(boyfriend);
				remove(boyfriend);
			}
			else if (SONG.song.toLowerCase() == 'kaboom')
			{
				dad = new Character(100, 100, 'sonic.exe alt');
				add(dad);
				remove(dad);
				
				boyfriend = new Boyfriend(100, 100, 'bf-pixel');
				add(boyfriend);
				remove(boyfriend);
				...
				gf = new Character(100, 100, 'gf-pixel');
				add(gf);
				remove(gf);
			}
		...
		if (!PlayStateChangeables.Optimize)
		{
			switch (stageCheck)
			{
				...
				case 'sonicexeStage': // i fixed the bgs and shit!!! - razencro part 1
					{
						...
						var bgcolor:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('SonicP2/solidcolor'));
						bgcolor.screenCenter();
						bgcolor.antialiasing = true;
						bgcolor.scrollFactor.set(1, 1);
						bgcolor.active = false;
						bgcolor.scale.x = 3.5;
						bgcolor.scale.y = 3.5;
						add(bgcolor);
						
						ScrollingBG = new FlxSprite(-1750, -400);
						ScrollingBG.frames = Paths.getSparrowAtlas('SonicP2/bgscroll', 'exe');
						ScrollingBG.animation.addByPrefix('go', 'bgscroll', 24);
						ScrollingBG.animation.addByPrefix('stop', 'scrollstop', 24);
						ScrollingBG.animation.play('go');
						ScrollingBG.antialiasing = true;
						ScrollingBG.scrollFactor.set(1, 1);
						ScrollingBG.scale.x = 1.3;
						ScrollingBG.scale.y = 1.3;
						add(ScrollingBG);
						
						P2sSKY = new FlxSprite(-414, -440.8).loadGraphic(Paths.image('SonicP2/sky'));
						P2sSKY.antialiasing = true;
						P2sSKY.scrollFactor.set(1, 1);
						P2sSKY.active = false;
						P2sSKY.scale.x = 1.4;
						P2sSKY.scale.y = 1.4;
						add(P2sSKY);

						P2trees = new FlxSprite(-290.55, -298.3).loadGraphic(Paths.image('SonicP2/backtrees'));
						P2trees.antialiasing = true;
						P2trees.scrollFactor.set(1.1, 1);
						P2trees.active = false;
						P2trees.scale.x = 1.2;
						P2trees.scale.y = 1.2;
						add(P2trees);

						P2bg2 = new FlxSprite(-306, -334.65).loadGraphic(Paths.image('SonicP2/trees'));
						P2bg2.updateHitbox();
						P2bg2.antialiasing = true;
						P2bg2.scrollFactor.set(1.2, 1);
						P2bg2.active = false;
						P2bg2.scale.x = 1.2;
						P2bg2.scale.y = 1.2;
						add(P2bg2);

						P2bg = new FlxSprite(-309.95, -240.2).loadGraphic(Paths.image('SonicP2/ground'));
						P2bg.antialiasing = true;
						P2bg.scrollFactor.set(1.3, 1);
						P2bg.active = false;
						P2bg.scale.x = 1.2;
						P2bg.scale.y = 1.2;
						add(P2bg);

						bgspec = new FlxSprite(-428.5 + 50 + 700, -449.35 + 25 + 392 + 105 + 50).loadGraphic(Paths.image("SonicP2/GangGarrison"));
						...
						mercs = new FlxSprite(-1600, 100);
						mercs.frames = Paths.getSparrowAtlas('SonicP2/mercs', 'exe');
						mercs.animation.addByPrefix('hm', 'heavy medic', 24);
						mercs.animation.addByPrefix('es', 'engineer spy', 24);
						mercs.animation.addByPrefix('ss', 'soldier sniper', 24);
						mercs.animation.addByPrefix('sp', 'scout pyro', 24);
						mercs.antialiasing = true;
						mercs.visible = false;
						mercs.scrollFactor.set(1.25, 1);
						add(mercs);
						
						var fade:FlxSprite = new FlxSprite(0, 1000).loadGraphic(Paths.image('SonicP2/blackfade'));
						...
						codes = new FlxSprite(0, 0).loadGraphic(Paths.image('SonicP2/soundtestcodes'));
						codes.antialiasing = true;
						codes.screenCenter();
						codes.scrollFactor.set(0, 0);
						codes.active = false;
						codes.scale.x = 1;
						codes.scale.y = 1;
						codes.alpha = 0;
				...
				case 'kaboomStage': // wow!! bbpanzu cool
					{
						defaultCamZoom = .9;
						curStage = 'kaboomStage';
						...
						var P2bgK:FlxSprite = new FlxSprite(-309.95, -240.2).loadGraphic(Paths.image('SonicP2/ground_kaboom'));
						P2bgK.antialiasing = true;
						P2bgK.scrollFactor.set(1.3, 1);
						P2bgK.active = false;
						P2bgK.scale.x = 1.2;
						P2bgK.scale.y = 1.2;
						add(P2bgK);

						bgspec = new FlxSprite(-428.5 + 50 + 700, -449.35 + 25 + 392 + 105 + 50).loadGraphic(Paths.image("SonicP2/KaboomPixel"));
						bgspec.antialiasing = false;
						bgspec.scrollFactor.set(1, 1);
						bgspec.active = false;
						bgspec.visible = false;
		...
		if (curStage == 'SONICstage' || curStage == 'SONICexestage' || curStage == 'kaboomStage') // i fixed the bgs and shit!!! - razencro part 2
		{
		...
		switch (curStage)
		{
			...
			case 'kaboomStage': // i fixed the bgs and shit!!! - razencro part 3
		...
		if (!PlayStateChangeables.Optimize)
		{
			...
			if (curStage == 'SONICstage' || curStage == 'SONICexestage' || curStage == 'FAKERSTAGE' || curStage == 'EXEStage' || curStage == 'kaboomStage')
				add(gf);
		...
		else if (curSong.toLowerCase() == 'kaboom')
		{
			FlxG.camera.follow(camFollow, LOCKON, 0.06 * (30 / (cast(Lib.current.getChildAt(0), Main)).getFPS()));
		}
		...
		switch (dad.curCharacter)
		{
			...
			case 'sonic', 'sonic.exe', 'faker', 'sunky':
				healthBar.createFilledBar(FlxColor.fromRGB(36, 132, 235), FlxColor.fromRGB(49, 176, 209));
		...
		switch (StringTools.replace(curSong, " ", "-").toLowerCase())
			{
				...
				case 'you-cant-run':
					...
					startCircle.x += 777;
					add(startCircle);
					startDemo.loadGraphic(Paths.image('StartScreens/Demo-ChargeStart'));
					startDemo.x -= 1200;
					add(startDemo);
					...
					{
						FlxTween.tween(startDemo, {x: 0}, 1, {ease: FlxEase.backOut});
					});

					new FlxTimer().start(4.2, function(tmr:FlxTimer)
					{
						FlxTween.tween(startCircle, {alpha: 0}, 1);
						FlxTween.tween(startDemo, {alpha: 0}, 1);
						FlxTween.tween(startText, {alpha: 0}, 1);
						FlxTween.tween(blackFuck, {alpha: 0}, 1);
					});
				case 'kaboom':
					add(blackFuck);
					startCircle.loadGraphic(Paths.image('StartScreens/CircleKaboom')); 
					startCircle.x += 777;
					add(startCircle);
					startText.loadGraphic(Paths.image('StartScreens/TextKaboom'));
					startText.x -= 1200;
					add(startText);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			if (daSection == 34 && curSong.toLowerCase() == 'kaboom')
				SONG.noteStyle = 'pixel';
				
			if (daSection == 50 && curSong.toLowerCase() == 'kaboom')
				SONG.noteStyle = 'normal';
				
			if (daSection == 66 && curSong.toLowerCase() == 'kaboom')
				SONG.noteStyle = 'pixel';
		...
		for (i in 0...deezNuts[ballsinyojaws])
		{
			...
			switch (noteTypeCheck)
			{
				case 'pixel':
					...
					babyArrow.setGraphicSize(Std.int(babyArrow.width * 6.5));
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					if (dad.curCharacter == 'sonic.exe' && curSong == 'you-cant-run')
					{
	...
	function goodNoteHit(note:Note, resetMashViolation = true):Void
	{
		...
		if (note.noteType == 4) // probably an easier way to do this but uhhh, fuck you, yeah, like the integer name
		{
			var fuckyou:Int = 0;
			heatlhDrop += 0.00025;
			if (heatlhDrop == 0.00025)
	...
	override function stepHit()
	{
		...
		if (curSong == 'you-cant-run')
		{
			...
			switch (curStep) // laugh zooms cuz why not
			{
				case 128, 328:
					defaultCamZoom = 1.1;
				case 142, 336:
			...
			switch (curStep) // me when bf do the chuckle
			{
				case 1288:
			...
			switch (curStep) // best part of the song aka demo beats the shit outa eryone
			{
				case 1174, 1238, 1304, 1366:
					dad.playAnim('pan1', true);
					dad.nonanimated = true;
				case 1206, 1270, 1334, 1398:
					dad.playAnim('pan2', true);
					dad.nonanimated = true;
				case 1184, 1216, 1248, 1280, 1314, 1344, 1376, 1408:
					dad.nonanimated = false;
			...
			switch (curStep) // best part of the song aka demo beats the shit outa eryone
			{
				case 1174, 1304:
					mercs.visible = true;
					mercs.animation.play('hm');
				case 1206, 1334:
					mercs.visible = true;
					mercs.animation.play('es');
				case 1238, 1366:
					mercs.visible = true;
					mercs.animation.play('ss');
				case 1270, 1398:
					mercs.visible = true;
					mercs.animation.play('sp');
			...
			switch (curStep) // pixel zooms (for pan because im lazy and dont wanna animate more pixels)
			{
				case 536, 568, 600, 632, 664, 696, 728, 760:
					defaultCamZoom = 1.1;
					FlxG.camera.shake(0.01, 0.15);
				case 540, 572, 604, 636, 668, 700, 732, 764:
					defaultCamZoom = 1.4;
					FlxG.camera.shake(0.02, 0.15);
				case 544, 576, 608, 640, 672, 704, 736, 768:
					defaultCamZoom = .9;

			}
			
			switch (curStep) // other pan zooms
			{
				case 1176, 1208, 1240, 1272, 1304, 1336, 1368, 1400:
					defaultCamZoom = .9;
					FlxG.camera.shake(0.01, 0.15);
				case 1180, 1212, 1244, 1276, 1308, 1340, 1372, 1404:
					defaultCamZoom = 1.2;
					FlxG.camera.shake(0.02, 0.15);
				case 1184, 1216, 1248, 1280, 1312, 1344, 1376, 1408:
					defaultCamZoom = .7;
			...
			else if (curStep == 784) // BACK TO NORMAL MF!!!
			{
				...
				remove(gf);
				gf = new Character(635.5 - 50 - 100, 265.1 - 250, 'gf-blue');
				add(gf);
			...
			if (curStep == 80) // la dumb swizik code lol
			{
				FlxG.camera.flash(FlxColor.fromRGB(15, 133, 230), 1);
			...
			if (curStep == 1160) // la dumb swizik code lol
			{
				defaultCamZoom = 1.5;
			}
			if (curStep == 1168) // holy shit they are faast
			{
				defaultCamZoom = .7;
				
				FlxG.camera.flash(FlxColor.fromRGB(15, 133, 230), 0.5);
				
				remove(boyfriend);
				boyfriend = new Boyfriend(1036 - 100, 300, 'bf-running');
				add(boyfriend);
				
				remove(dad);
				dad = new Character(116, 0, 'demo-charge');
				add(dad);
				
				FlxTween.tween(gf, {x: -2500, y: 0}, .5);
				
				P2sSKY.visible = false;
				P2bg.visible = false;
				P2bg2.visible = false;
				P2trees.visible = false;
			}
			if (curStep == 1424) // bf is fucking dead L
			{
				boyfriend.playAnim('fall', true);
				boyfriend.nonanimated = true;
				FlxTween.tween(boyfriend, {y: 2000}, 5, { ease: FlxEase.cubeInOut });
				
				ScrollingBG.animation.play('stop');
			}
			if (curStep == 1430) // ayo demo u dont wanna fall off too
			{
				dad.playAnim('stop', true);
				dad.nonanimated = true;
			}
			if (curStep == 1440) // ayo demo u dont wanna fall off too
			{
				FlxTween.tween(boyfriend, {alpha: 0}, 1.5);
				FlxTween.tween(camHUD, {alpha: 0}, 1.5);
			}
			if (curStep == 1454) // ayo demo u dont wanna fall off too
			{
				FlxTween.tween(codes, {alpha: 1}, 5);
			}
		}
		if (curSong == 'kaboom')
		{
			...
			switch (curStep) // switchin to and fro pixel state
			{
				case 272:			
					FlxG.camera.flash(FlxColor.fromRGB(15, 133, 230), 1);
