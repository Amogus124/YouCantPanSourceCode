...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		logoBlBUMP.frames = Paths.getSparrowAtlas('demo logo bop');
		logoBlBUMP.animation.addByPrefix('idle', "logo bop", 24);
		logoBlBUMP.animation.addByPrefix('enter', "logo enter", 24);
		logoBlBUMP.animation.play('idle');
	...
	override function update(elapsed:Float)
	{
		...
		if (pressedEnter && !transitioning && skippedIntro && code != 4)
		{
			...
			if (FlxG.save.data.flashing)
				...
				logoBlBUMP.animation.play('enter');
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			FlxG.sound.play(Paths.sound('showMoment', 'shared'), .7);
